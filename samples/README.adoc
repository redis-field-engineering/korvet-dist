...
=== link:logstash-tiered-pipeline/README.adoc[Logstash Tiered Pipeline]

End-to-end local pipeline using Docker Compose with Redis, Korvet, and two Logstash instances.

**Includes:**

* `redis:latest` for hot-tier storage
* `redisfield/korvet:early-access` for Kafka-compatible streaming
* Logstash producer generating synthetic logs into `logs`
* Logstash consumer enriching events into `logs-processed`
* Topic recreation with tiered-storage settings matching `eks-pipeline`
* Local filesystem cold storage under `./data/cold`

**Use Cases:**

* Local reproduction of the EKS pipeline flow
* Logstash producer/consumer Kafka compatibility testing
* Tiered storage demos without S3 or Kubernetes
...
All samples use the official `redisfield/korvet:early-access` Docker image from Docker Hub - no build step required!
...
Docker will automatically pull `redisfield/korvet:early-access` on first run. If you see this error, try:

[source,bash]
----
docker pull redisfield/korvet:early-access
----
