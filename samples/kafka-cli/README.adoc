...
* ✅ Kafka protocol compatibility - no code changes needed
* ✅ Producer sending messages via `kafka-console-producer`
* ✅ Consumer reading messages via `kafka-console-consumer`
* ✅ Data stored in Redis Streams
* ✅ Zero code required - just CLI commands
...
NOTE: Wait 10-15 seconds for services to initialize. Docker will automatically pull the `redisfield/korvet:early-access` image on first run.
...
Watch messages appear instantly in Terminal 1! 🎉
...
# Inside Redis CLI:
KEYS "korvet:storage:local:*"                          # List partition streams
XLEN "korvet:storage:local:demo-topic:0"               # Count messages
XRANGE "korvet:storage:local:demo-topic:0" - + COUNT 10  # Read messages
----
...
[NOTE]
====
Korvet stores record keys and values as opaque bytes, exactly like Kafka. There is no server-side payload format or validation, so *any* content works — JSON, plain text, Avro, Protobuf, or arbitrary binary.

The examples in this sample use JSON only because it is easy to read in the console:
...
Plain strings such as `Hello` are equally valid.
====
...
* ✅ Java/Python/Go/Node.js Kafka clients
* ✅ Kafka Connect
