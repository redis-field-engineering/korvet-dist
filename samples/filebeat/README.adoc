...
XLEN "korvet:storage:local:filebeat-logs:0"                # Count messages
XRANGE "korvet:storage:local:filebeat-logs:0" - + COUNT 10 # Read first 10 messages
XINFO GROUPS "korvet:storage:local:filebeat-logs:0"        # Check consumer groups
----
...
. **Redis** (Storage) stores the messages in a stream with key `korvet:storage:local:filebeat-logs:0`
...
The sample uses `redisfield/korvet:early-access` from Docker Hub. Docker will automatically pull it on first run. If you see this error, try:
...
docker-compose exec redis redis-cli XINFO GROUPS "korvet:storage:local:filebeat-logs:0"
