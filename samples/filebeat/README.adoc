...
# Inside Redis CLI:
# New topics use segmented storage: messages live in per-segment streams keyed
# "korvet:storage:local:filebeat-logs:0:<segmentId>" (one per rolled segment),
# not the single "...:filebeat-logs:0" key.
KEYS "korvet:storage:local:*"                                # List all topic/segment streams
XLEN "korvet:storage:local:filebeat-logs:0:0"                # Count messages in segment 0
XRANGE "korvet:storage:local:filebeat-logs:0:0" - + COUNT 10 # Read first 10 messages of segment 0
XINFO GROUPS "korvet:storage:local:filebeat-logs:0:0"        # Check consumer groups on segment 0
----
...
. **Redis** (Storage) stores the messages in per-segment streams keyed `korvet:storage:local:filebeat-logs:0:<segmentId>` (segmented storage is the default for new topics)
...
The sample uses `redisfield/korvet:latest` from Docker Hub. Docker will automatically pull it on first run. If you see this error, try:
...
# Verify consumer group exists (segment 0; segmented storage is the default)
docker-compose exec redis redis-cli XINFO GROUPS "korvet:storage:local:filebeat-logs:0:0"
