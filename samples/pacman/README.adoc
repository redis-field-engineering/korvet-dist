...
✅ **Korvet masquerading as Kafka** - Standard Kafka clients work without code changes +
✅ **Event-driven** - Pac-Man game events streamed in real-time +
✅ **Producer & Consumer** - See events flowing through Korvet +
✅ **Redis backend** - All data stored in Redis Streams +
...
* ✅ Pull the official `redisfield/korvet:early-access` image from Docker Hub
* ✅ Start Redis, Korvet, web server, and consumer
* ✅ Start Kafka UI for browser-based topic inspection
* ✅ Start RedisInsight for direct stream inspection
* ✅ Install Node.js dependencies automatically
* ✅ Make everything available in ~30 seconds
...
[source,text]
----
🎮 USER_GAME: {"user":"player123","game":{"score":100,"lives":3,"level":1}}
💀 USER_LOSSES: {"user":"player123"}
🎮 USER_GAME: {"user":"player456","game":{"score":250,"lives":2,"level":2}}
----

=== Browse Topics In The UI

Open http://localhost:8081 to inspect topics, messages, and consumer groups in your browser.

This Kafka UI service runs inside Docker and connects to Korvet over `korvet:9092`.

NOTE: Topic and cluster metadata work reliably here. If message browsing shows fetch errors, use RedisInsight below to inspect the underlying Redis streams directly.

=== Browse Messages In RedisInsight

Open http://localhost:5540 for a reliable browser view of Korvet's backing Redis data.

In RedisInsight, connect to:
...
Korvet stores topic data in Redis Streams such as:

* `korvet:storage:local:USER_GAME:0`
* `korvet:storage:local:USER_LOSSES:0`
...
docker compose logs -f kafka-ui # View Kafka UI logs
docker compose logs -f redisinsight # View RedisInsight logs
...
1. **Game produces events** when you:
   * Score points → `USER_GAME` event
   * Lose a life → `USER_GAME` event
   * Game over → `USER_LOSSES` event
...
* The sample uses the Docker-network broker address: `korvet:9092`
...
* Korvet writes to Redis Stream: `XADD korvet:storage:local:USER_GAME:0 * value <json>`
* Korvet sends ProduceResponse back to client

**Consume Request:**

* Client sends Kafka FetchRequest
* Korvet reads from Redis Stream: `XREAD STREAMS korvet:storage:local:USER_GAME:0 <offset>`
...
[source,text]
----
Key: korvet:storage:local:USER_GAME:0
Type: Stream
Entries: [
...
# Inside Redis CLI:
KEYS "korvet:storage:local:*"                      # List all topics
XLEN "korvet:storage:local:USER_GAME:0"            # Count messages in USER_GAME
XRANGE "korvet:storage:local:USER_GAME:0" - + COUNT 10  # Read last 10 messages
----
...
----
samples/pacman/
├── docker-compose.yml       # All services (Redis, Korvet, web, consumer)
├── package.json             # Node.js dependencies
├── server.js                # API server (replaces Lambda)
├── consumer.js              # Event consumer
├── public/                  # Pac-Man game files
│   ├── index.html
│   ├── game/
│   │   ├── js/
│   │   │   ├── game.js      # Modified to call local API
│   │   │   ├── shared.js    # Modified API endpoint
│   │   │   └── ...
│   │   ├── css/
│   │   └── img/
│   └── ...
...
[source,text]
----
❌ Failed to connect to Korvet: Connection refused
----
...
XLEN "korvet:storage:local:USER_GAME:0"
----

4. Check consumer is subscribed to correct topics:
   * Should see: `📡 Subscribed to topic: USER_GAME`
   * Should see: `📡 Subscribed to topic: USER_LOSSES`
...
# List all topics
docker compose exec redis redis-cli KEYS "korvet:storage:local:*"

# Count messages in a topic
docker compose exec redis redis-cli XLEN "korvet:storage:local:USER_GAME:0"

# Read messages from a topic
docker compose exec redis redis-cli XRANGE "korvet:storage:local:USER_GAME:0" - + COUNT 10
...
* ✅ Standard Kafka producer/consumer APIs
* ✅ kafkajs (Node.js)
* ✅ kafka-python
* ✅ Java Kafka clients
* ✅ Filebeat, Logstash, Spark Streaming
...
* Provides automatic hot/cold data tiering (Redis → Apache Iceberg on S3)
