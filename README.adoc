...
docker run -d -p 9092:9092 -p 8080:8080 \
  -e KORVET_REDIS_URI=redis://host.docker.internal:6379 \
  redisfield/korvet
----
...
* link:samples/load-testing/README.adoc[*Load Testing*] - Performance testing with Prometheus and Grafana observability
* link:samples/filebeat/README.adoc[*Filebeat Sample*] - Log aggregation with Filebeat and Logstash
* link:samples/kafka-connect/README.adoc[*Kafka Connect Sample*] - Local Kafka Connect smoke test writing Korvet events to a file
...
Korvet is a commercial product of Redis Ltd.

* **Evaluation use** (development, testing) is permitted
* **Production use** requires a commercial license from Redis
* This repository contains documentation and samples only — not the Korvet source code

For licensing, contact Redis (https://redis.io/) or your Redis account representative.
